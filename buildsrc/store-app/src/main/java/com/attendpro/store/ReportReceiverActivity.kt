...
class ReportReceiverActivity : Activity() {
    ...
    private fun buildUi(){
        ...
        root.addView(UiKit.heroCard(this,p,10).apply{
            addView(UiKit.title(this@ReportReceiverActivity,p,t("هاتف الاستلام وإدارة المحل","Receiver & Store Management"),24f).apply{gravity=Gravity.CENTER;setTextColor(android.graphics.Color.WHITE)})
        ...
        listOf(t("التفعيل","Activation"),t("التقارير","Reports"),t("إدارة المحل","Store management")).forEachIndexed{i,s->tabs.addView(UiKit.button(this,p,s,tab==i).apply{setOnClickListener{tab=i;buildUi()}})}
        root.addView(tabs)
    ...
    private fun management(root:LinearLayout){
        ...
        if(!receiver.canMessageEmployees&&!receiver.canManageStore)root.addView(UiKit.card(this,p,9).apply{addView(UiKit.sectionLabel(this@ReportReceiverActivity,p,t("إدارة المحل","Store management")));addView(UiKit.subtitle(this@ReportReceiverActivity,p,t("لا توجد صلاحيات إدارة ممنوحة لهذا الهاتف.","No management permissions are granted to this phone.")))})
    }
