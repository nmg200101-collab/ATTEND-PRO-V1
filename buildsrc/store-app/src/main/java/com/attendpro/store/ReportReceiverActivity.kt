...
class ReportReceiverActivity : Activity() {
    ...
    private fun refreshRemote(silent:Boolean){
        ...
        Thread {
            ...
            runOnUiThread{
                if(!silent) {
                    if(r.isSuccess) info(t("تم التحديث ✓","Updated ✓"),t("تم تحديث الصلاحيات والحالة من الخادم.","Permissions and status were refreshed from the server."))
                    else info(t("تعذر التحديث","Refresh failed"),receiverErrorMessage(r.exceptionOrNull()))
                }
                buildUi()
            }
        }.start()
    ...
    private fun openEmployees(){
        Thread {
            ...
            runOnUiThread{
                if(r.isFailure){
                    info(t("إدارة الموظفين","Employee management"),receiverErrorMessage(r.exceptionOrNull()))
                    return@runOnUiThread
                }
                val es=r.getOrThrow()
                val labels=mutableListOf(t("＋ إضافة موظف","＋ Add employee"))
                labels.addAll(es.map{
                    val state = if(it.enabled)t("نشط","Active") else t("موقوف","Disabled")
                    val pending = if(it.pendingCommand) t(" • قيد التنفيذ"," • Pending") else ""
                    val link = if(it.pendingLink) t(" • الهاتف غير مرتبط"," • Phone not linked") else ""
                    "${it.employeeName} • ${it.employeeId} • $state$pending$link"
                })
                AlertDialog.Builder(this)
                    .setTitle(t("إدارة الموظفين","Employee management"))
                    .setMessage(t("أي تعديل يُرسل كأمر آمن إلى جهاز المحل ويُطبق عند اتصاله بالخادم.","Changes are queued securely and applied by the authorized Store device when it is online."))
                    .setItems(labels.toTypedArray()){_,i->
                        if(i==0) editEmployee(null)
                        else {
                            val employee=es[i-1]
                            if(employee.pendingCommand) {
                                info(t("الأمر قيد التنفيذ","Command pending"),t("يوجد أمر سابق لهذا الموظف ينتظر تطبيق جهاز المحل.","A previous command for this employee is waiting for the Store device to apply it."))
                            } else employeeActions(employee)
                        }
                    }
    ...
    private fun editEmployee(e:ReceiverEmployeeAdminClient.Employee?){
        ...
        val d=AlertDialog.Builder(this)
            ...
            .setMessage(t("سيُرسل التغيير إلى جهاز المحل المعتمد، ولن يغيّر اقتران هاتف الموظف أو Bluetooth/GPS.","The change is sent to the authorized Store device and never changes employee-phone pairing or Bluetooth/GPS."))
            .setView(box)
        ...
        d.setOnShowListener{
            d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{
                ...
                if(eid.isBlank()||n.length<2){
                    info(t("بيانات غير مكتملة","Incomplete data"),t("أدخل رقم الموظف واسمه بشكل صحيح.","Enter a valid employee ID and name."))
                    return@setOnClickListener
                }
                Thread{
                    val r=if(e==null) ReceiverEmployeeAdminClient.add(receiver.serverUrl,receiver.receiverId,receiver.secret,eid,n,b)
                    else ReceiverEmployeeAdminClient.update(receiver.serverUrl,receiver.receiverId,receiver.secret,eid,n,b)
                    runOnUiThread{
                        if(r.isSuccess){
                            d.dismiss()
                            info(t("تم إرسال الأمر ✓","Command sent ✓"),t("سيطبقه جهاز المحل تلقائيًا عند اتصاله بالخادم.","The Store device will apply it automatically when connected to the server."))
                            openEmployees()
                        } else info(t("تعذر إرسال الأمر","Unable to send command"),receiverErrorMessage(r.exceptionOrNull()))
                    }
                }.start()
            }
        }
    ...
    private fun employeeActions(e:ReceiverEmployeeAdminClient.Employee){
        AlertDialog.Builder(this)
            ...
            .setItems(arrayOf(t("تعديل","Edit"),if(e.enabled)t("إيقاف الموظف","Disable employee")else t("تفعيل الموظف","Enable employee"))){_,i->
                if(i==0) editEmployee(e)
                    ...
                    runOnUiThread{
                        if(r.isSuccess){
                            info(t("تم إرسال الأمر ✓","Command sent ✓"),t("سيتم تطبيق حالة الموظف على جهاز المحل عند المزامنة.","The employee status will be applied on the Store device during sync."))
                            openEmployees()
                        } else info(t("تعذر التحديث","Update failed"),receiverErrorMessage(r.exceptionOrNull()))
                    }
                }.start()
            }
            .setNegativeButton(t("إلغاء","Cancel"),null)
    ...
    private fun receiverErrorMessage(error:Throwable?):String{
        val raw=error?.message.orEmpty()
        return when{
            raw.contains("Failed to connect",true) || raw.contains("Network is unreachable",true) || raw.contains("ENETUNREACH",true) ->
                t("تعذر الوصول إلى الخادم عبر الشبكة الحالية. سيستخدم التطبيق IPv4 الاحتياطي تلقائيًا عند إعادة المحاولة.","Unable to reach the server on this network. The app will automatically use the IPv4 fallback on retry.")
            raw.contains("resolve",true) || raw.contains("UnknownHost",true) ->
                t("تعذر حل عنوان الخادم. سيجرب التطبيق DNS الآمن ثم عناوين IPv4 الاحتياطية.","Unable to resolve the server. The app will try secure DNS and then verified IPv4 fallback addresses.")
            raw.startsWith("HTTP 403") -> raw.removePrefix("HTTP 403: ").ifBlank{t("الصلاحية غير ممنوحة لهذا الهاتف.","This permission is not granted to this phone.")}
            raw.startsWith("HTTP 404") -> raw.removePrefix("HTTP 404: ").ifBlank{t("الخدمة المطلوبة غير متاحة على الخادم.","The requested service is unavailable on the server.")}
            raw.isBlank() -> t("تعذر الاتصال بالخادم. أعد المحاولة بعد لحظات.","Unable to contact the server. Try again shortly.")
            else -> raw
        }
    }
